...
**Physical note on sharing modes.** In a post-hoc aggregate, instantaneous netting is **conservation-inherent**: a community has one connection point, so simultaneous surplus and deficit physically net (you cannot suppress P2P matching while surplus and deficit coexist). Hence the two meaningful, non-degenerate behaviours are *netting only* (`p2p`) and *netting + a shared battery on the residual* (`community_battery`/`both`). The config exposes all three of the user's named modes; `community_battery` and `both` are operationally identical in v1 (see §4 + Open Q1).
...
- **All three modes via one path.** `p2p` ⟹ `battery=None` (self-consumption with no store: `cg_exp = net_surplus`, `cg_imp = net_deficit`). `community_battery`/`both` ⟹ a community `Battery`; netting still emerges from the self-consumption stage (conservation). The PV-only-home path already exercises `simulate_timestep(battery=None, …)` (`home.py:249-297`), so this is a proven reuse.
...
```yaml
community:
  sharing_mode: both              # "p2p" | "community_battery" | "both"
  community_battery:              # required for community_battery/both; forbidden for p2p
    capacity_kwh: 50.0
    max_charge_kw: 20.0
...
| Sharing model | `sharing_mode ∈ {p2p, community_battery, both}`; netting always applied (conservation), community battery optional | All three user-named modes exposed. `community_battery`≡`both` operationally in a post-hoc model (netting is conservation-inherent) — documented; see Open Q1. *(user-confirmed: P2P, community battery, or both)* |
...
### β — community battery layer (community_battery / both)
- **Modules:** `community.py` (+ `tests/unit/test_community.py`)
- **Work:** extend `simulate_community` so `community_battery`/`both` instantiate a `Battery(config.community_battery)` and pass it into the **same** reused `simulate_timestep` call; record `cb_ch`/`cb_dis`/`soc`. No new dispatch logic.
- **Prereqs:** α (intra-batch, serialises `community.py`).
- **Verification signal:** unit test — with a community battery, a net-surplus step charges it (SOC rises, bounded by `max_charge_kw`/capacity via the reused `Battery`), a later net-deficit step discharges it to meet deficit (`cg_imp` falls vs the `p2p`-only result); `validate_community_balance` holds every step including the `(cb_ch − cb_dis)` term; `both` and `community_battery` produce identical flows.
...
1. **`community_battery` vs `both` semantics.** In a post-hoc aggregate, instantaneous netting is conservation-inherent, so these two modes are operationally identical in v1 (both = netting + battery). The enum keeps all three of your named modes for config clarity and future divergence (e.g. a battery-only mode that *defers* netting under a different dispatch policy). **Confirm this collapse is acceptable**, or we split them by giving the battery a distinct (non-self-consumption) dispatch — revisit in α/β if you disagree.
2. **Community battery default size for the demo scenario.** δ must pick a `capacity_kwh`/power for `scenarios/bristol-community.yaml` that visibly stores midday surplus into the evening peak for the chosen fleet. Tune empirically in δ; the field is configurable either way.
3. **`p2p` netting granularity.** v1 nets at the fleet-aggregate level (`total_grid_export` vs `total_grid_import`) — correct for a single community connection point. A future multi-feeder / sub-community topology would net per-group; out of scope for v1, revisit if topology config appears.
