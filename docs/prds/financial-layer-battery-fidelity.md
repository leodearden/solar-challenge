...
class FinanceConfig:
    ...
    #   loan_term_years>0; loan_rate>=0; asset_life_years>=loan_term_years; positive costs.
...
- **Capex build-up** per home from the resolved fleet configs: `Σ_home (pv_kwp×pv_cost_per_kwp + roof_fit + battery_kwh×battery_cost_per_kwh)`; total minus `grant` is financed `equity_fraction` / `1-equity_fraction`.
...
| **W3** (install-config optimisation sweep, sibling — *not yet authored*) | **produces / consumed-by** | W3 imports `finance.householder_bill` / `project_economics` / `project_multi_year` to rank configs by min householder bill with project-surplus as a constraint | **P6 owns** the function signatures (the contract, §3.1/§8); W3 references the P6 task as a dependency | seam defined here; W3 to be authored |
...
No reciprocal-ownership ambiguity: W3 does not exist yet to claim the seam, and the **user-observable report surface** (`solar-challenge finance run`) is itself a named consumer, so G1 is satisfied independent of W3.
