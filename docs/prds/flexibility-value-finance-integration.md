...
- **The W2 financial layer has landed** (`finance.py`: `householder_bill`, `project_multi_year`, `project_economics`, `spreadsheet_revenue_curve`; θ calibration task #48 `done`). `FinanceConfig` lives at `config.py:478`. The **W2 cost-recovery amendment** (`docs/prds/cost-recovery-householder-billing.md`) adds the flex field; it landed mid-authoring with a per-kWh full-flex shape that was **reconciled** to the grid-services-only-per-home split this PRD assumes (§8; `e791214`, Leo 2026-06-17).
