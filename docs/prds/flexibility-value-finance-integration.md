...
- **The W2 financial layer has landed** (`finance.py`: `householder_bill`, `project_multi_year`, `project_economics`, `spreadsheet_revenue_curve`; θ calibration task #48 `done`). `FinanceConfig` lives at `config.py:478`. The **W2 cost-recovery amendment** (`docs/prds/cost-recovery-householder-billing.md`) adds the flex field; it landed mid-authoring with a per-kWh full-flex shape that is **being re-opened** to the grid-services-only-per-home split this PRD assumes (§8, resolved with Leo 2026-06-17).
...
- **Blocked-on-consumer (out-of-batch dependency):** task **δ** (grid-services parameter → economics) depends on the **W2 cost-recovery amendment** exposing the `FinanceConfig` **grid-services** field + wiring it into the economics. The amendment landed (`docs/prds/cost-recovery-householder-billing.md`, `2e39a03`) but with a *per-kWh full-flex* field that **conflicts** with this PRD's A3 split; per Leo (2026-06-17) the resolution is to **keep W1's physics and re-open the W2 amendment** to a grid-services-only-per-home field (§8). δ is queued behind that **re-opened** field — do not queue δ until it lands.
...
1. **Final `FinanceConfig` field name** (now a re-open, not a green-field add). The W2 amendment shipped `flex_income_per_battery_kwh_per_year_gbp` (per-kWh, full-flex); the resolved seam (Leo 2026-06-17) re-opens it to `grid_services_income_per_battery_home_gbp` (per-home, grid-services-only). **Blocking dependency, not tactical:** the W2 re-open is a required gated pass before δ can queue. Final name is W2's call; W1 references the agreed name.
