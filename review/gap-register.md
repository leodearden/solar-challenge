...
- **Amended 2026-06-16 (W3/P7 dependency):** tasks **#43 (γ)** + **#47 (η)** gain `FinanceConfig.inverter_cost_per_kw_gbp` (**default 0.0**, calibration-preserving) + the `Σ inverter_kw×cost` term in the `project_economics` capex build-up, so W3 can price inverter sizing. Default 0 ⟹ existing finance YAMLs + the [FIN] calibration (θ/H6) stay **bit-identical**. PRD §3.1/§3.4/§4/§7 + manifest §γ/§η updated.
...
- **PRD file:** `docs/prds/discrete-install-config-sweep.md` (committed `6b9611a`). **Not yet decomposed** — queue-gated on W2 tasks **#43/#44/#46/#47** (γ/δ/ζ/η) reaching done/merged (incl. the inverter-capex amendment). 5-task plan (A enumerate → B run/rank → C Pareto → D sensitivity → E report+CLI) in PRD §10.
