...
### P6 — Financial layer + battery fidelity (W2)  ·  Status: **QUEUED**
...
- **PRD file:** `docs/prds/financial-layer-battery-fidelity.md` (committed `54bbdda`) · manifest `docs/prds/financial-layer-battery-fidelity.capability-manifest.md` (`54bbdda`) · **Subtasks queued** (decompose 2026-06-16, `planning_mode`): **#41** (α — BatteryConfig SOC/eff) → **#42** (β — battery SOH; dep #41) ; **#43** (γ — FinanceConfig) → **#44** (δ — householder_bill + report + CLI; dep #43, **leaf**) ; **#45** (ε — FleetSummary financial agg) ; **#46** (ζ — project_multi_year; dep #42+#45) ; **#47** (η — project_economics + report; dep #43+#46+**#44**, **leaf**) ; **#48** (θ — spreadsheet calibration; dep #44+#47, **integration-gate leaf**). Intra-batch edges: β→α, δ→γ, ζ→β,ε, η→γ,ζ,**δ**, θ→δ,η — the **η→δ** edge (output.py report-scaffold ordering) added at decompose per manifest §η detail table (user-confirmed; the manifest's batch-verdict summary line had omitted it). Cross-PRD consumes **#2** / **P3** (#16/#17) / **P5** (#30–#34) all **done** — referenced as context, no new edges. Manifest's one PASS-conditional binding resolved: `scipy.interpolate.PchipInterpolator` confirmed importable (`uv run --extra dev`, scipy 1.17.1) — no fallback needed.
- **Amended 2026-06-16 (W3/P7 dependency):** W2's finance contract gains `FinanceConfig.inverter_cost_per_kw_gbp` (**default 0.0**, calibration-preserving) + a `Σ inverter_kw×cost` term in the `project_economics` capex build-up, so W3 can price inverter sizing. Default 0 ⟹ existing finance YAMLs + the [FIN] calibration (θ/H6) stay **bit-identical**. PRD §3.1/§3.4/§4/§7 + manifest §γ/§η updated. **Implementation:** filed as **follow-on task #49** (γ/#43 was already `done`/in-progress at amend time, and in-progress tasks don't pick up spec edits — so η/#47 builds the *original* capex formula and #49 adds the field + inverter term afterward). **#49 deps:** #43 (γ, done) + #47 (η).

### P7 — Discrete install-config optimisation sweep (W3)  ·  Status: **PRD-DRAFTED** (queue-gated on W2)
- **Source:** workstream **W3** from the 2026-06-15 deployment-readiness survey §6 decision 6 / §7 / §8. Consumes P6/W2's financial layer; not a 2026-05-30-review gap.
- **Gap:** no capability to rank the discrete install menu (`PV × battery × inverter`) by minimum householder bill with project surplus as a hard Pareto constraint (£27/home/yr floor) + an assumption-sensitivity panel. Existing sweep machinery ranges one distribution multiplier, not a discrete cartesian product; `output.py` has no cross-config ranking/Pareto/sensitivity report.
- **Owns (new):** `src/solar_challenge/optimize.py` (pure enumerate/rank/Pareto/sensitivity), `output.generate_config_ranking_report` (additive on `output.py`, serialised after P6's finance sections), `src/solar_challenge/cli/optimize.py` (`solar-challenge optimize configs`) + one `add_typer` line in `cli/main.py`.
- **Consumes (don't modify):** P6/W2 `finance.householder_bill`/`bill_distribution`/`project_multi_year`/`project_economics`/`FinanceConfig` (incl. the `inverter_cost_per_kw_gbp` term added per this PRD); `fleet.simulate_multi_sweep_iter`/`simulate_fleet`/`FleetResults.home_configs`; frozen config dataclasses + `dataclasses.replace`; `PVConfig.inverter_capacity_kw` (clipping live in `simulate_pv_output`).
- **Decided (user-confirmed):** **homogeneous-install fleet per config** (clean per-config surplus + bill distribution straight from W2, zero attribution math); primary rank = **year-1 net annual householder bill**; surplus = `net_surplus_per_home_per_year_gbp` per config, **£27 floor flagged** (Pareto, never scalarised); **inverter swept** (G98 3.68 kW default) + priced via the P6 amendment above; **OAT sensitivity** over `FinanceConfig` knobs. Approach **B + targeted H** (W3↔W2 seam boundary test; W3 does not re-implement economics).
- **PRD file:** `docs/prds/discrete-install-config-sweep.md` (committed `6b9611a`). **Not yet decomposed** — queue-gated on W2 tasks **#44/#46/#47** (δ/ζ/η) + inverter-capex **#49** reaching done/merged. 5-task plan (A enumerate → B run/rank → C Pareto → D sensitivity → E report+CLI) in PRD §10.
- **Decompose trigger (deterministic, human-attention-free):** **task #50** — an L2/human-required escalation *gate* task (no code) depending on **#44/#46/#47/#48 + #49**. When W2's finance layer + the inverter-capex follow-on have all landed, #50 becomes runnable and immediately escalates to L2 asking a human to run `/prd` decompose on the W3 PRD. No human has to watch for W2 completion; the trigger fires on the dependency graph.
...
| `output.py` finance report sections (`generate_finance_report`) | **P6** | disjoint from **P5**'s `generate_community_report` and #2's SEG summary line; **P7** adds `generate_config_ranking_report` (disjoint, serialised after P6) |
| `FinanceConfig.inverter_cost_per_kw_gbp` + `project_economics` inverter capex term | **P6** (added 2026-06-16 per W3) | **P7 (W3)** consumes to price inverter sizing; default `0.0` keeps W2 calibration bit-identical |
| **`optimize.py`** (new pure enumerate/rank/Pareto/sensitivity) + `cli/optimize.py` (new) + `output.generate_config_ranking_report` | **P7 (W3)** | — · imports P6 `finance.py` read-only (never edits it); user surface = `solar-challenge optimize configs` · PRD `docs/prds/discrete-install-config-sweep.md` |
